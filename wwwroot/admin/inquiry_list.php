...
// 「sort順番jを変える時」用のURIパラメタの作成
// XXX foreachを、本当はまとめると、より良い
$awk = array();
...
if ('' !== $find_string['name']) {
	...
	$name_e =
		str_replace(array('\\',		'%',	'_'),
					...
					$find_string['name']);
	//
	$bind_data[':name'] = "{$find_string['name']}%"; // 性能重視
...
// SQLを実行
$r = $pre_count->execute(); // XXX エラーチェック
// レコード件数を取得
$rec_num = $pre_count->fetchAll();
$rec_num = $rec_num[0][0];
//var_dump($rec_num);

// 最大page数の計算(全体要素数/1Page要素数:端数切り上げ)
$max_page_num = ceil($rec_num / $per_page) - 1;
//var_dump($max_page_num);

// 「前のPage」と「前のPage」のページ数を設定
